...
public class SimpleHibernateDao<T, ID extends Serializable> {
	...
	/**
	 * 采用@Inject按类型注入SessionFactory, 当有多个SesionFactory的时候在子类重载本函数.
	 */
	@Inject
	...
	public List<T> queryBy(final String propertyName, final Object value) {
		Validate.notBlank(propertyName, "propertyName不能为空");
		return find(createCriteria(Restrictions.eq(propertyName, value)).addOrder(Order.desc("id")));
	}
